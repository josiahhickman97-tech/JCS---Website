# Rebuild all 5 service detail pages with unique, problem-led content.
...
<head>
  <!-- Google tag (gtag.js) -->
  <script async src="https://www.googletagmanager.com/gtag/js?id=AW-18397273274"></script>
  <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());
    gtag('config', 'AW-18397273274');
  </script>
  <meta charset="UTF-8" />
  ...
  <script type="application/ld+json">
  {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": "Solutions", "item": "https://jcssafetysystems.com/solutions" },
...
$(Build-Zones $cfg.zones)
    ...
    <section class="section section-alt" id="solutions">
      <div class="container">
        <div class="section-header">
          <span class="section-label">Capabilities and pairings</span>
...
$(Build-Faq $cfg.faqs)
    ...
    <section class="section section-sm">
      <div class="container">
        <div class="section-header">
          <span class="section-label">More solutions</span>
          <h2 class="section-title">Explore other solutions</h2>
        </div>
...
"@
  ...
  $path = Join-Path $root "services\$($cfg.slug).html"
  [System.IO.File]::WriteAllText($path, $html, [System.Text.UTF8Encoding]::new($false))
  Write-Host "Wrote $($cfg.slug).html ($((Get-Item $path).Length) bytes)"
}

. (Join-Path $root "_solutions-content.ps1")

foreach ($cfg in $SolutionPages) {
  Write-IndustryPage $cfg
}

Write-Host "`nValidation:"
Get-ChildItem (Join-Path $root "services\*.html") | ForEach-Object {
